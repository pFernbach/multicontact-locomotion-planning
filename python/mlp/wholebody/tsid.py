...
def adjustEndEffectorTrajectoryIfNeeded(cfg, phase, robot, data, eeName, effectorMethod):
    ...
    if not current_placement.isApprox(ref_placement, 1e-1):
...
def generate_wholebody_tsid(cfg, cs_ref, fullBody=None, viewer=None):
    ...
    urdf = package_path + cfg.Robot.packageName.lstrip(package_name) + '/urdf/' + cfg.Robot.urdfName + cfg.Robot.urdfSuffix + '.urdf'
